...
EVENT_USE_CASES = {
    ...
    "view_item": {
        ...
        "tag": "revenue",
        "title": "Where Product Views Become Sales",
        "narrative": "view_item is the truest read on standalone product demand — one fire per shopper landing on a specific PDP, before cart or checkout muddies the signal. The pagePath breakdown ranks exactly which products pull the most views, separating genuine bestsellers from PDPs that draw traffic but never move product. Layer the conversion-lift comparison on top: pages with heavy views but view-to-purchase performance trailing the site-wide baseline point straight at pricing, imagery, or stock issues on that specific PDP, while pages beating baseline are the merchandising template worth replicating — reallocate ad spend and homepage real estate toward the latter and flag the former for a price test or out-of-stock check.",
        "secondary": "item",
        "secondary_param": "",
    },
    "view_item_list": {
        ...
        "tag": "revenue",
        "title": "Collection Merchandising Priority Map",
        "narrative": "Every collection grid load is a merchandising bet — 23,142 users have taken it, but only 7.5% convert, so the real question isn't whether the event fires, it's which collections earn that traffic and which just burn it. The pagePath breakdown ranks collections by view volume; cross-reference the top-traffic pages against the conversion-lift comparison to spot the two failure modes worth acting on: a high-traffic collection converting below baseline (a dead-end grid eating homepage/nav real estate that should be re-merchandised or retired) and a low-traffic collection with strong lift (a hidden winner worth promoting into primary nav or homepage placement). Reorder collection priority around what the data shows converts, not what currently gets the clicks.",
        "secondary": "item",
        "secondary_param": "",
    },
    "add_to_cart": {
        "category": "Cart", "tag": "revenue",
        "title": "Cart Abandonment Recovery Signal",
        "narrative": "add_to_cart is the loudest purchase-intent signal GA4 has short of a completed order, and the live numbers show the gap it's leaving on the table: 6,485 users triggered it, only 29.3% went on to purchase, meaning roughly seven in ten shoppers who add a product then walk away. The page-path breakdown pinpoints exactly which product pages generate the most adds, so a page piling up add_to_cart volume while trailing the site-wide conversion-lift baseline isn't a merchandising win -- it's a specific product with a checkout-friction or price-hesitation problem. That combination is the trigger to launch cart abandonment email/retargeting or a checkout UX audit targeted at that product, converting already-expressed intent into revenue instead of losing it to silent abandonment.",
        "secondary": "item",
        "secondary_param": "",
    },
    "view_cart": {
        "category": "Cart",
        "tag": "revenue",
        "title": "The High-Intent Cart Checkpoint",
        "narrative": "view_cart was a genuine gap before go-live — it never fired pre-pixel, so this mid-funnel signal was invisible. It's a stronger intent marker than add_to_cart alone: these are shoppers who came back to actually look at what's in the cart, not just impulse-added an item and moved on. With 1,848 users triggering it and 39.1% going on to purchase — well above site-wide baseline — the page-path breakdown of which pages feed into this step becomes the map of where that high-intent traffic originates. If conversion from view_cart ever trails what add_to_cart volume would predict, that gap points straight at cart-page friction — a shipping cost surprise, no visible discount field, unclear next-step CTA — worth fixing to turn more of this already-warm intent into completed orders.",
        "secondary": "item",
        "secondary_param": "",
    },
    "begin_checkout": {
        "category": "Checkout", "tag": "revenue",
        "title": "The 43% Checkout Leak",
        "narrative": "begin_checkout is the highest-intent moment on the site short of purchase itself, and live numbers expose exactly how much value leaks right after it fires: 3,426 users started checkout but only 56.6% went on to purchase, meaning roughly 43% of shoppers who already committed to buying dropped out anyway. That gap sits entirely inside checkout itself -- payment options, shipping-cost transparency in EGP, form length, lack of guest checkout -- not in acquisition, since every one of these users is already warm and costing zero incremental ad spend. The page-breakdown chart shows which pages feed the most checkout starts, pinpointing where to concentrate friction-reduction work first, while the conversion-lift comparison quantifies exactly how far checkout-starters trail a fully-converted baseline. Recovering even a fraction of that 43% gap is pure incremental revenue -- reason to prioritize checkout UX above nearly anything else on the roadmap.",
        "secondary": "item", "secondary_param": "",
    },
    "remove_from_cart": {
        "category": "Cart Behavior",
        "tag": "cost",
        "title": "Per-Product Second-Guess Detector",
        "narrative": "remove_from_cart is GA4's core ecommerce event, but unlike most events in this dashboard it's a negative-intent signal -- a shopper actively pulling an item back out of the basket rather than adding or browsing further. Live data shows the conversion-lift comparison holds up well: of the 2,678 users who triggered it, 39.1% still went on to purchase, well above what a true funnel-exit signal would produce, meaning most removals are single-item cart curation or second-guessing, not a shopper walking away for good. The real signal is in the per-product breakdown -- cross-referencing removal volume against each product's add-to-cart volume isolates the outliers with a disproportionately high removal-to-add ratio, flagging a specific product-level problem (price shock at checkout, sizing doubt, a PDP overselling the item) worth investigating on that SKU rather than treating it as a whole-funnel issue.",
        "secondary": "item",
        "secondary_param": "",
    },
    "add_shipping_info": {
        "category": "Checkout", "tag": "revenue",
        "title": "Shipping-Fee Friction Costs Checkouts",
        "narrative": "add_shipping_info was a real blind spot before this pixel went live — a core GA4 ecommerce event that simply never fired, hiding the exact step where shipping-cost sticker shock kills checkouts. Live data already shows why it matters: 3,311 users triggered it and 57.8% went on to purchase, a very high rate since this fires one step before add_payment_info — so any meaningful drop between the two events' lift rates is a direct, quantified read on shipping-fee friction (an unexpected fee or missing free-shipping threshold), not vague funnel decay. The page breakdown shows exactly which entry points feed this step, telling you where to surface shipping cost or a free-shipping threshold earlier (e.g. on the cart page) before the shopper ever reaches this point. Given these are the closest-to-purchase users site-wide, closing that add_shipping_info-to-add_payment_info gap is worth prioritizing over almost any other funnel fix.",
        "secondary": "item", "secondary_param": "",
    },
    "add_payment_info": {
        "category": "Checkout",
        "tag": "revenue",
        "title": "The Last Six Percent",
        "narrative": "This is the final gate before purchase, so the 94.2% who complete after triggering it are basically already sold — that leaves a thin ~6% gap that isn't hesitation, it's friction: a declined card, a Paymob or COD hiccup, a gateway timeout. The page breakdown shows which checkout step and payment path feed this event, pinpointing where the failure actually happens. If the conversion lift here already sits near the ceiling versus baseline, stop treating this as a UX problem — route the fix to payment gateway reliability and retry flows, since intent is locked in and every recovered session is close to pure incremental revenue.",
        "secondary": "item",
        "secondary_param": "",
    },
}
...
META_AUDIENCE_USE_CASES = {
    ...
    "view_item": {
        ...
        "mapped_standard_event": "ViewContent",
        "audience_use_case": "ViewContent is Meta's core product-retargeting signal, powering Dynamic Product Ads that show shoppers the exact item they viewed instead of generic creative. Tier the audience by recency — a 1-3 day window for hot, high-intent browsers who haven't converted yet, a 7-30 day window for broader nurture retargeting — and exclude anyone with a recent Purchase event so spend isn't wasted on customers who already bought. High-value product viewers (filtered by viewed item price) also make a strong seed audience for Lookalikes, surfacing new shoppers who browse similar price points and categories.",
        "why_this_event": "ViewContent is one of Meta's most reliable, highest-volume ecommerce signals, and Meta's own optimization models lean on it heavily for both prospecting and retargeting. Worth noting: the Meta Pixel currently runs via Shopify's native Web Pixel Manager, separate from the GTM custom pixel capturing this event, so the mapping is ready the moment that gap gets wired up.",
    },
    "view_item_list": {
        "eligible": True,
        "meta_event_type": "custom",
        "mapped_standard_event": "",
        "audience_use_case": "Feed view_item_list (with the collection/category name in event params) via CAPI as a custom event, and build a broader, softer top-of-funnel Custom Audience of \"Category Browsers\" -- shoppers who viewed a specific collection grid but never reached a product page. This audience sits a notch above generic site traffic and below ViewContent in intent, making it useful for warming up cold prospects with category-level creative before narrowing to product-level retargeting. It's also a solid input signal for a shopping-interest Lookalike, since collection browsing reveals category affinity (e.g. dresses vs. activewear) that a Purchase-only seed audience is too thin to capture.",
        "why_this_event": "Meta's own docs reserve ViewContent for a single product detail view, not a category/listing grid, so view_item_list has no direct standard-event match -- it has to ride as a custom event, distinct enough from ViewContent to warrant its own broader top-of-funnel audience rather than being folded into it.",
    },
    "add_to_cart": {
        "eligible": True,
        "meta_event_type": "standard",
        "mapped_standard_event": "AddToCart",
        "audience_use_case": "Maps straight to Meta's AddToCart standard event, one of the highest-ROAS signals in the whole Meta ecosystem once wired through (these custom pixel events don't reach Meta yet -- separate from the native Shopify Web Pixel). The core play is dynamic cart-abandonment retargeting: show the exact product added, run it in a tight 1-3 day hot window while intent is still fresh, layer in a steep discount or urgency creative to force the decision, and exclude anyone who already fired Purchase so budget isn't wasted on converted customers. It also doubles as an optimization event for Conversions campaigns when raw Purchase volume is too thin for Meta's algorithm to optimize against directly.",
        "why_this_event": "It captures a concrete, product-specific commitment past passive browsing, which is exactly the mid-funnel intent Meta's algorithm and dynamic retargeting creative need to work -- far stronger and more actionable than ViewContent, and available in volume well before the thinner Purchase event.",
    },
    "view_cart": {
        "eligible": True,
        "meta_event_type": "custom",
        "mapped_standard_event": "",
        "audience_use_case": "As a custom event, view_cart won't populate Ads Manager's Purchase or AddToCart conversion dropdowns automatically, but it can still seed a Custom Audience once passed through Pixel or CAPI. It sits as the middle tier in a three-stage retargeting ladder — AddToCart (warm), view_cart (hot), InitiateCheckout (hottest) — letting ad spend concentrate on shoppers who came back to actually review their cart rather than everyone who ever added a single item. That's a sharper, higher-intent segment than plain AddToCart retargeting, worth its own bid tier or creative angle (shipping-cost reassurance, discount nudge) once wired up — these custom pixel events don't reach Meta yet, since Meta Pixel runs via Shopify's native Web Pixel Manager separately from the GTM setup capturing them.",
        "why_this_event": "No standard event covers the moment a shopper returns to actually check their cart contents between AddToCart and InitiateCheckout, but that behavior itself is a strong purchase-intent filter — good enough to justify a dedicated hot-tier audience despite missing native Ads Manager conversion treatment.",
    },
    "begin_checkout": {
        "eligible": True, "meta_event_type": "standard", "mapped_standard_event": "InitiateCheckout",
        "audience_use_case": "Maps to InitiateCheckout, which builds the hottest retargeting tier Tharaa can create short of Purchase itself -- checkout abandoners hit within a 1-3 day window with urgency or limited-time discount creative convert at a premium over any earlier-funnel segment. As a Conversions-objective optimization event it gives Meta's delivery algorithm a strong, high-volume signal to chase when raw Purchase volume is too thin to optimize against reliably. It also seeds better lookalikes than AddToCart alone -- users who reached checkout showed near-terminal commercial intent, so a lookalike built from this pool skews closer to genuine buyers than one built from earlier browse or cart signals.",
        "why_this_event": "begin_checkout maps directly to Meta's InitiateCheckout, one of the strongest non-Purchase signals in the whole funnel, so it inherits native Custom Audience, Conversions-campaign, and lookalike support the moment it's wired to Meta -- right now these events only reach the GTM custom pixel, not Shopify's native Web Pixel Manager, so none of it hits Meta yet.",
    },
    "remove_from_cart": {
        "eligible": False,
        "meta_event_type": "custom",
        "mapped_standard_event": "",
        "audience_use_case": "remove_from_cart has no Meta standard-event equivalent and, more importantly, is a directionally ambiguous trigger -- a removal can mean genuine disinterest, a size/variant swap, a cart cleanup, or a change of mind on one line item, and GA4 can't distinguish which at the event level. The live numbers back that ambiguity: 39.1% of the 2,678 users who fired it still converted, well above a \"lost cause\" baseline, so seeding a Custom Audience or exclusion rule from it would suppress or \"win back\" a segment that was mostly going to buy anyway. This event carries more value as an internal product-analytics input -- surfacing which SKUs get reconsidered -- than as a Meta audience-building signal.",
        "why_this_event": "Removing an item doesn't reliably map to falling purchase intent -- the 39.1% still-converts rate shows most removers are curating their cart, not abandoning it -- so using it as a negative-intent signal for retargeting or suppression would misclassify a majority-converting segment as cold.",
    },
    "add_shipping_info": {
        "eligible": True, "meta_event_type": "custom", "mapped_standard_event": "",
        "audience_use_case": "Meta has a standard AddPaymentInfo event but no AddShippingInfo equivalent — a real taxonomy gap — so this fires as a custom event, but it's arguably the hottest audience signal on the site, sitting one step from Purchase and past InitiateCheckout/AddPaymentInfo territory. Once wired to Meta (currently none of these GTM custom-pixel events reach Meta yet, since Meta Pixel runs separately via Shopify's native Web Pixel Manager), it should seed its own ultra-short 0-2 day Custom Audience for abandoner retargeting, reserving the most aggressive urgency and incentive creative (limited-time discount, free-shipping unlock, countdown) for this cohort rather than folding them into a broader cart/checkout audience. It's also a strong secondary optimization event for Conversions campaigns: sitting this close to Purchase, it gives Meta's algorithm a denser, faster-arriving signal to optimize against in accounts where raw Purchase volume is too thin to train reliably.",
        "why_this_event": "Confirming a shipping method is about as close to Purchase as a non-payment event gets, and Meta's own taxonomy has no dedicated slot for it — the closest analog, AddPaymentInfo, only covers the next step down. That gap makes this custom event more valuable than it looks: it isolates the exact cohort worth the highest bid multiplier and tightest retargeting window on the whole site.",
    },
    "add_payment_info": {
        "eligible": True,
        "meta_event_type": "standard",
        "mapped_standard_event": "AddPaymentInfo",
        "audience_use_case": "Once wired up, this builds the narrowest and hottest non-Purchase Custom Audience on the site — shoppers who reached the payment step and didn't convert, the closest thing to a buyer Meta lets you retarget. It earns the most aggressive urgency creative and the shortest retargeting window of any tier (same-day/next-day, decaying fast after that), with near-mandatory exclusion of anyone who purchased since triggering it to avoid wasting spend on already-closed sales. As a Conversions-objective optimization event it's exceptionally strong given the observed 94.2% completion rate — Meta's algorithm is optimizing toward a signal that's already almost Purchase itself.",
        "why_this_event": "AddPaymentInfo maps 1:1 to Meta's own standard event of the same name, the single hottest pre-Purchase signal in Meta's taxonomy, since a user entering payment details is one step from converting.",
    },
}
